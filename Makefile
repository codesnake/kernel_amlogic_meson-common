...
RECOVERY_RAMFS_SOURCE := $(patsubst "%root", "%recovery/root", $(CONFIG_INITRAMFS_SOURCE))
MY_RAMFS_SOURCE := $(CONFIG_INITRAMFS_SOURCE)
ifeq ($(BUILD_URECOVERY), true)
	MY_RAMFS_SOURCE := $(RECOVERY_RAMFS_SOURCE)
endif

ifneq ($(KBUILD_SRC),)
	MY_RAMFS_SOURCE := $(patsubst "%", "$(KBUILD_SRC)/%", $(MY_RAMFS_SOURCE))
endif
endif

PHONY += $(vmlinux-dirs)
$(vmlinux-dirs): prepare scripts
	$(Q)$(MAKE) $(build)=$@ CONFIG_INITRAMFS_SOURCE="$(MY_RAMFS_SOURCE)"
